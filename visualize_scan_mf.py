...
def visualize_1d(c_portion: float, if_norm: bool = True):
    ...
    plt.title(f"Nano_mf_5d_1c-{c_portion:.2%}", fontsize=fontsize)
    if if_norm:
        base_x = torch.linalg.vector_norm(cbo_factory.x_tensor_range, dim=-1)
    else:
        base_x = cbo_factory.x_tensor_range
    plt.scatter(
        base_x.squeeze().to(device="cpu").numpy(),
        cbo_factory.y_tensor.squeeze().to(device="cpu").numpy(),
        c="black",
        s=1,
        label="Objective value (out of feasible region)",
    )
    ...
    plt.scatter(
        feasible_x.squeeze().to(device="cpu").numpy(),
        feasible_y.squeeze().to(device="cpu").numpy(),
        c="purple",
        s=1,
        label="Objective value (inside feasible region)",
    )
    plt.scatter(
        base_x[cbo_factory.max_arg].to(device="cpu").numpy(),
        cbo_factory.y_tensor[cbo_factory.max_arg].to(device="cpu").numpy(),
        c="red",
        s=100,
        marker="*",
        label="Optimum",
...
for idx, c_portion in enumerate(np.linspace(0.1, 0.9, 5)):
    ...
    try:
        RES_num["CMES-IBO"] = np.load(
            f"{BASELINE_DIR}OL-Regret-Figure_NANO_MF_5D_1C-CP{c_portion:.2%}-InterP-RI1--none-cmes-ibo-R{n_repeat}-P2-T{n_iter}_I1_L4-TI1-USexact.npy"
        )
    except:
        pass
    try:
        RES_num["cEI"] = np.load(
            f"{BASELINE_DIR}OL-Regret-Figure_NANO_MF_5D_1C-CP{c_portion:.2%}-InterP-RI1--none-qei-R{n_repeat}-P2-T{n_iter}_I1_L4-TI1-USexact.npy"
        )
    except:
        pass
    try:
        RES_num["SCBO"] = np.load(
            f"{BASELINE_DIR}OL-Regret-Figure_NANO_MF_5D_1C-CP{c_portion:.2%}-InterP-RI1--none-scbo-R{n_repeat}-P2-T{n_iter}_I1_L4-TI1-USexact.npy"
        )
    except:
        pass
    ...
    ax.set_title(f"Nano_mf_5d_1c-{c_portion:.2%} Simple Regret")
    if idx == 0:
...
plt.savefig("simple_regret_scan_nano.pdf")
