...
def experiment(exp:str='rastrigin_1d', method:str='qei', n_repeat:int=2, train_times:int=5, n_iter:int=20, n_init:int=10, 
    ...
    if exp == 'rastrigin_1d': # rastrigin 1D
        ...
        scbo = 'scbo' in method
        if scbo:
            x_tensor, y_func, c_func_list = cbo_factory.RE2_3D_5C(scbo_format=scbo)
        else:
            x_tensor, y_tensor, c_tensor_list = cbo_factory.RE2_3D_5C(scbo_format=scbo)
        constraint_threshold_list, constraint_confidence_list = cbo_factory.constraint_threshold_list, cbo_factory.constraint_confidence_list
        feasible_filter = cbo_factory.feasible_filter
        y_tensor = cbo_factory.y_tensor
        cbo_factory.visualize_1d(if_norm=True)
...
if __name__ == "__main__":
    ...
    #     # experiment(exp='water_converter_32d_neg_3c', n_init=n_init3, n_repeat=2, n_iter=2, method=method, )
    #     experiment(exp='water_converter_32d_neg_3c', n_init=n_init3, n_repeat=n_repeat, n_iter=200, method=method, )


    experiment(exp="vessel_4D_3C", n_init=2, n_iter=200, n_repeat=15, method='cmes-ibo')
    # for method in METHODs:
    #     # if method in ['cbo', 'cmes-ibo']:
    #     if method in ['ts', 'random']:
    #         continue
